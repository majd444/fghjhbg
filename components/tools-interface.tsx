...
export function ToolsInterface({ onToolSelect, selectedToolId, componentId, workflowComponents, workflowConnections }: ToolsInterfaceProps) {
  ...
  useEffect(() => {
    ...
    if (authSuccess === 'success' && service && authComponentId === componentId) {
      console.log('Authentication success detected for service:', service);
      ...
      // Find the matching tool and select it
      const toolToSelect = tools.find(tool => tool.service === service);
      if (toolToSelect && onToolSelect) {
        console.log('Auto-selecting tool after authentication:', toolToSelect.name);
        onToolSelect(toolToSelect);
        setSelectedTools([toolToSelect.id]);
      }
    }
  }, [searchParams, componentId, onToolSelect])
