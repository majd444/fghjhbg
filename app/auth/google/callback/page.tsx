  ...
  useEffect(() => {
    async function handleCallback() {
      try {
        ...
        console.log('Auth callback received:', { code: code?.substring(0, 10) + '...', state, error });
        ...
        // Extract service and log all state data for debugging
        ...
        // Set client-side cookie for auth status (for UI purposes)
        document.cookie = `is_authenticated=true; path=/; max-age=${30 * 24 * 60 * 60}; SameSite=Strict`;
        
        // Store auth status in localStorage too as a backup
        localStorage.setItem('is_authenticated', 'true');
        
        // Check if we have a redirect URL from the response
        const redirectTo = data.redirect || '/';
        
        // This will ensure the root path is hit, which will then route based on payment status
        setTimeout(() => {
